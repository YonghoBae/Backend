...
//아주 간단한 계산함수 만들기
//fnPlus(a,b)함수에 전달되는 a,b파라메터(매개변수)를 받아 로직을 처리함
//결과값을 반환하는 함수
function fnPlus(a,b){
    let c = a+b;
    //콘솔로깅 전용함수를 호출해서 콘솔에 계산결과값을 출력
    fnLogging(c);
    return c;
}

//특정함수를 매개변수로 전달해서 해당 함수내 특정 위치에서 콜백함수가 실행되게 구현
//콜백함수 개념이해
//콜백함수를 이용하면 특정함수내에서 순차적 프로그래밍이 가능
//매개변수 함수를 다양한 함수로 변경해서 전달
...
//자바스크립트 함수는 특정함수의 파라메터(매개변수)로 전달가능
//fnLogging()함수를 콜백함수로 전달
const result1 = fnPlusCallBack(1000,2000,fnLogging);

//콜백함수를 파라메터 내에서 직접 정의전달
//fnPlusCallBack(1000,2000,fnLogging)과 동일한 방식
...
//콜백함수를 구현(사용)하는 이유는 기본함수가 실행될때 기본함수 내에서
